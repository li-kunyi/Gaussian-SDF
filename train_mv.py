...
from gaussian_renderer import sdf_render_v2, sdf_render_v3, network_gui, get_sdf_loss_with_gaussian_depth, get_mv_loss
import sys
...
def render(opt, iteration, scene, viewpoint_cam, gaussians, pipe, background, dataset):
    ...
    # gaussian_sdf_loss = torch.abs(gaussian_sdf[gaussian_sdf>0]).mean()
    loss += opt.lambda_gaussian_sdf * gaussian_sdf_loss

    if opt.lambda_gradient_consistency > 0:
        _normal = render_normal_world if iteration >= opt.densify_until_iter else None
        consistency_loss = gradient_consistency_loss(depth_grad_mag, dx, dy, gt_image, image, normal=_normal)
        loss += opt.lambda_gradient_consistency * consistency_loss  
    ...
    with torch.no_grad():
        ...
        if is_save_images and (iteration % 100 == 0):
            ...
            torchvision.utils.save_image(image_to_show, f"{dataset.model_path}/{TIMESTAMP}log_images/{iteration}_{viewpoint_cam.idx}.jpg")
            
    return render_pkg, loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, save_ckpt, ckpt_pth, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        camera_idx1 = randint(0, len(viewpoint_stack)-1)
        viewpoint_cam1 = viewpoint_stack[camera_idx1]
        camera_idx2 = 0 if camera_idx1 == (len(viewpoint_stack) - 1) else camera_idx1 + 1
        viewpoint_cam2 = viewpoint_stack[camera_idx2]
        
        # Pick a random high resolution camera
        if random.random() < 0.3 and dataset.sample_more_highres:
            camera_idx1 = randint(0, len(highresolution_index)-1)
            viewpoint_cam1 = trainCameras[highresolution_index[camera_idx1]]
            camera_idx2 = 0 if camera_idx1 == (len(highresolution_index) - 1) else camera_idx1 + 1
            viewpoint_cam2 = trainCameras[highresolution_index[camera_idx2]]
            
        # Render
        if (iteration - 1) == debug_from:
            pipe.debug = True

        render_pkg1, loss1 = render(opt, iteration, scene, viewpoint_cam1, gaussians, pipe, background, dataset)
        rendering1 = render_pkg1["render"]
        depth1 = rendering1[6, :, :]
        alpha1 = rendering1[7, :, :]
        
        render_pkg2, loss2 = render(opt, iteration, scene, viewpoint_cam2, gaussians, pipe, background, dataset)
        rendering2 = render_pkg2["render"]
        depth2 = rendering2[6, :, :]
        alpha2 = rendering2[7, :, :]
        mv_loss = get_mv_loss(viewpoint_cam1, depth1, alpha1, viewpoint_cam2, depth2, alpha2)

        loss = loss1 + loss2 + 0.1 * mv_loss
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                viewspace_point_tensor1, visibility_filter1, radii1 = render_pkg1["viewspace_points"], render_pkg1["visibility_filter"], render_pkg1["radii"]
                gaussians.max_radii2D[visibility_filter1] = torch.max(gaussians.max_radii2D[visibility_filter1], radii1[visibility_filter1])
                gaussians.add_densification_stats(viewspace_point_tensor1, visibility_filter1, render_pkg1['frustum_mask'])

                viewspace_point_tensor2, visibility_filter2, radii2 = render_pkg2["viewspace_points"], render_pkg2["visibility_filter"], render_pkg2["radii"]
                gaussians.max_radii2D[visibility_filter2] = torch.max(gaussians.max_radii2D[visibility_filter2], radii2[visibility_filter2])
                gaussians.add_densification_stats(viewspace_point_tensor2, visibility_filter2, render_pkg2['frustum_mask'])

                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    size_threshold = 20 if iteration > 30000 else None
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.05, scene.cameras_extent, size_threshold, iteration, z_prune=opt.z_prune)
        ...
        if iteration < opt.iterations:
            ...
            if iteration > opt.start_train_sdf:
                torch.cuda.empty_cache()
                with torch.no_grad():
                    render_pkg1 = sdf_render_v3(viewpoint_cam1, gaussians, pipe, background, kernel_size=dataset.kernel_size)
                    rendering1 = render_pkg1["render"]
                    depth1 = rendering1[6, :, :]
                    alpha1 = rendering1[7, :, :]

                    render_pkg2 = sdf_render_v3(viewpoint_cam2, gaussians, pipe, background, kernel_size=dataset.kernel_size)
                    rendering2 = render_pkg2["render"]
                    depth2 = rendering2[6, :, :]
                    alpha2 = rendering2[7, :, :]

                n_inner_iter = opt.n_inner_iter
                for inner_iter in range(n_inner_iter):
                    ...
                    n_sample_surface = 21 if iteration < opt.densify_until_iter else opt.n_sample_surface
                    lambda_sdf = 1.0 if iteration < opt.densify_until_iter else opt.lambda_sdf
                    lambda_fs = 0. if iteration < opt.densify_until_iter else opt.lambda_fs

                    H, W = depth1.shape
                    fx = W / (2 * math.tan(viewpoint_cam1.FoVx / 2))
                    fy = H / (2 * math.tan(viewpoint_cam1.FoVy / 2))
                    c2w1 = (viewpoint_cam1.world_view_transform.T).inverse()
                    fs_loss1, sdf_loss1 = get_sdf_loss_with_gaussian_depth(gaussians, c2w1, fx, fy, 
                                                                                    depth1, alpha1,
                    ...
                    grid_loss1 = lambda_sdf * sdf_loss1 + lambda_fs * fs_loss1

                    c2w2 = (viewpoint_cam1.world_view_transform.T).inverse()
                    fs_loss2, sdf_loss2 = get_sdf_loss_with_gaussian_depth(gaussians, c2w2, fx, fy, 
                                                                                    depth2, alpha2,
                                                                                    n_pixel=n_pixel, n_sample=n_sample, n_sample_surface=n_sample_surface, 
                                                                                    truncation=opt.truncation,
                                                                                    full_image=False, ray_sampling=True)
                    grid_loss2 = lambda_sdf * sdf_loss2 + lambda_fs * fs_loss2
                    grid_loss = grid_loss1 + grid_loss2

                    if opt.lambda_smooth > 0 and iteration > opt.densify_until_iter:
                        grid_loss += opt.lambda_smooth * smoothness(gaussians, sample_points=opt.smooth_sample_point, voxel_size=opt.smooth_voxel_size)
                ...
                # with torch.no_grad():   
                #     tb_writer.add_scalar('train_loss_patches/fs_loss', fs_loss1.item(), iteration)
                #     tb_writer.add_scalar('train_loss_patches/sdf_loss', sdf_loss1.item(), iteration)     
        ...
        with torch.no_grad():
            ...
            if (iteration + 1) % 10 == 0:
                training_report(tb_writer, iteration+1, loss, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, sdf_render_v3, (pipe, background, dataset.kernel_size))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[5_000, 15_000, 20_000, 25_000, 30_000])
    ...
    parser.add_argument('--den_interval', type=int, default=100)
    ...
    op.densification_interval = args.den_interval
    args.densification_interval = args.den_interval
