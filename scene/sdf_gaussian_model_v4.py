...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        self.opacity = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.visible_mask = torch.zeros_like(self.opacity, dtype=torch.bool, device="cuda")
        ...
        l = [
            ...
            {'params': self.sdf2opacity.parameters(), 'lr': self.sdf2opacity_lr, "name": "beta"},
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "beta":
                lr = self.sdf2opacity_lr if iteration < 3000 else 0.0
                param_group['lr'] = lr
            # return lr
    ...
    def save_ply(self, path):
        ...
        sdf = self.query_sdf(self._xyz).detach().cpu().numpy()
    ...
    def densify_and_clone(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, use_sdf_mask=False):
        ...
        # new_sdf = self._sdf[selected_pts_mask]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        prune_mask = torch.logical_and(prune_mask, self.visible_mask.squeeze())
        ...
        self.sdf = self.query_sdf(self.get_xyz).squeeze()
        self.densify_and_clone(grads, max_grad, grads_abs, Q, extent, use_sdf_mask=True)
        clone = self._xyz.shape[0]
        
        self.densify_and_split(grads, max_grad, grads_abs, Q, extent, use_sdf_mask=True)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, frustum_opacity=None, frustum_mask=None):
        ...
        if frustum_mask is not None:
            self.opacity[frustum_mask] = frustum_opacity
            self.visible_mask[frustum_mask] = torch.ones_like(frustum_opacity, dtype=torch.bool, device="cuda")
            # self.visible_mask = frustum_mask