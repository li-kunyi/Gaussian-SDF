...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, ref_sh_degree: int):
        ...
        self._ref_features_dc = torch.empty(0)
        self._ref_features_rest = torch.empty(0)
    ...
    def get_ref_features(self):
        features_dc = self._ref_features_dc
        features_rest = self._ref_features_rest
        return torch.cat((features_dc, features_rest), dim=1)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        features[:, 3:, 1:] = 0.0
        ref_features = torch.zeros((fused_color.shape[0], 3, (self.max_ref_sh_degree + 1) ** 2)).float().cuda()
        ...
        self._ref_features_dc = nn.Parameter(ref_features[:, :, 0:1].transpose(1, 2).contiguous().requires_grad_(False))
        self._ref_features_rest = nn.Parameter(ref_features[:, :, 1:].transpose(1, 2).contiguous().requires_grad_(False))
    ...
    def construct_list_of_attributes(self, exclude_filter=False):
        ...
        for i in range(self._ref_features_dc.shape[1] * self._ref_features_dc.shape[2]):
            l.append('ref_f_dc_{}'.format(i))
        for i in range(self._ref_features_rest.shape[1] * self._ref_features_rest.shape[2]):
            l.append('ref_f_rest_{}'.format(i))
        l.append('opacity')
    ...
    def save_ply(self, path):
        ...
        ref_f_dc = self._ref_features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        ref_f_rest = self._ref_features_rest.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, ref_f_dc, ref_f_rest, opacities, scale, rotation, filter_3D), axis=1)
    ...
    def save_fused_ply(self, path):
        ...
        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, ref_f_dc, ref_f_rest, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def reset_opacity(self):
        ...
        current_opacity_with_filter = self.get_opacity#_with_3D_filter
    ...
    def load_ply(self, path):
        ...
        if "ref_f_dc_0" in plydata.elements[0].keys():
            ref_features_dc = np.zeros((xyz.shape[0], 3, 1))
            ref_features_dc[:, 0, 0] = np.asarray(plydata.elements[0]["ref_f_dc_0"])
            ref_features_dc[:, 1, 0] = np.asarray(plydata.elements[0]["ref_f_dc_1"])
            ref_features_dc[:, 2, 0] = np.asarray(plydata.elements[0]["ref_f_dc_2"])
            extra_ref_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("ref_f_rest_")]
            assert len(extra_ref_f_names) == 3 * (self.max_ref_sh_degree + 1) ** 2 - 3
            ref_features_extra = np.zeros((xyz.shape[0], len(extra_ref_f_names)))
            for idx, attr_name in enumerate(extra_ref_f_names):
                ref_features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
            # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
            ref_features_extra = ref_features_extra.reshape((ref_features_extra.shape[0], 3, (self.max_ref_sh_degree + 1) ** 2 - 1))
        else:
            ref_features_dc = np.zeros((xyz.shape[0], 3, 1))
            ref_features_extra = np.zeros((ref_features_extra.shape[0], 3, (self.max_ref_sh_degree + 1) ** 2 - 1))
    ...
    def prune_points(self, mask, opt_opacity=True):
        ...
        self._ref_features_dc = self._ref_features_dc[valid_points_mask]
        self._ref_features_rest = self._ref_features_rest[valid_points_mask]
        if opt_opacity:
            self._opacity = optimizable_tensors["opacity"]
        else:
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest,new_ref_features_dc, new_ref_features_rest, 
                              new_opacities, new_scaling, new_rotation, opt_opacity=True):
        if opt_opacity:
            d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, N=2, opt_opacity=True):
        ...
        new_ref_features_dc = self._ref_features_dc[selected_pts_mask].repeat(N, 1, 1)
        new_ref_features_rest = self._ref_features_rest[selected_pts_mask].repeat(N, 1, 1)
        ...
        # self.max_radii2D = torch.cat((self.max_radii2D, self.max_radii2D[selected_pts_mask].repeat(N)), dim=0)
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
    ...
    def densify_and_clone(self, grads, grad_threshold, grads_abs, grad_abs_threshold, scene_extent, sdf_gradient=None, opt_opacity=True):
        ...
        new_ref_features_dc = self._ref_features_dc[selected_pts_mask]
        new_ref_features_rest = self._ref_features_rest[selected_pts_mask]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, sdf_value=None, sdf_th=0.5, sdf_gradient=None, opt_opacity=True):
        ...
        if not sdf_gradient == None:
            sdf_gradient = sdf_gradient[~prune_mask]
