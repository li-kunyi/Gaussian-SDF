...
class MeshExtractor(object):
    def __init__(self, gaussians, render, pipe, bg_color=None, kernel_size=0.0):
        """
        a class that extracts attributes a scene presented by 2DGS
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        self.viewpoint_stack = viewpoint_stack
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            rendering = self.render(viewpoint_cam, self.gaussians)["render"]
    ...
    def estimate_bounding_sphere(self):
        ...
        poses = c2ws[:,:3,:] @ np.diag([1, -1, -1, 1])
        center = (focus_point_fn(poses))
        self.radius = np.linalg.norm(c2ws[:,:3,3] - center, axis=-1).max()
        self.center = torch.from_numpy(center).float().cuda()
        print(f"The estimated bounding radius is {self.radius:.2f}")
        print(f"Use at least {2.0 * self.radius:.2f} for depth_trunc")
    ...
    def extract_mesh_unbounded(self, resolution=1024):
        ...
        """
        def contract(x):
            mag = torch.linalg.norm(x, ord=2, dim=-1)[..., None] + 1e-8
            return torch.where(mag < 1, x, (2 - (1 / mag)) * (x / mag))
        
        def uncontract(y):
            mag = torch.linalg.norm(y, ord=2, dim=-1)[..., None] + 1e-8
            return torch.where(mag < 1, y, (1 / (2-mag) * (y/mag)))
        
        def compute_rgb_perframe(i, points, rgbmap, viewpoint_cam):
            """
                compute per frame sdf
            """
            new_points = torch.cat([points, torch.ones_like(points[...,:1])], dim=-1) @ viewpoint_cam.full_proj_transform
            ...
            mask_proj = ((pix_coords > -1. ) & (pix_coords < 1.) & (z > 0)).all(dim=-1)
            sampled_rgb = torch.nn.functional.grid_sample(rgbmap.cuda()[None], pix_coords[None, None], mode='bilinear', padding_mode='border', align_corners=True).reshape(3,-1).T
            return sampled_rgb, mask_proj
        
        def colorize(samples, inv_contraction):
            if inv_contraction is not None:
                samples = inv_contraction(samples)

            rgbs = torch.zeros((samples.shape[0], 3)).cuda()
            mask = torch.zeros((samples.shape[0])).bool().to(samples.device)

            weights = torch.ones_like(samples[:,0])
            for i, view in tqdm(enumerate(self.viewpoint_stack), desc="Colorization integration progress"):
                rgb, mask_proj = compute_rgb_perframe(i, samples,
                    rgbmap = self.rgbmaps[i],
                    viewpoint_cam=self.viewpoint_stack[i],
                )
                ...
                ones = torch.ones((samples.shape[0], 1), dtype=torch.float32, device="cuda")
                homo_points = torch.cat([samples, ones], dim=1).reshape(-1, 4, 1)
            ...
            return rgbs.cpu().numpy(), mask.cpu().numpy()
        
        # N = resolution
        # print(f"Computing sdf gird resolution {N} x {N} x {N}")
        
        # from utils.mcube_utils import marching_cubes_with_contraction
        # self.box_center = self.gaussians.query_sdf.box_center
        # self.box_length_half = self.gaussians.query_sdf.box_length_half
        # self.sigmoid_alpha = self.gaussians.query_sdf.sigmoid_alpha
        ...
        # def inverse_normalize(y):
        #     normalized_dist = -1 / self.sigmoid_alpha * torch.log((1 - y) / (y + 1e-10))
        #     x = normalized_dist * self.box_length_half + self.box_center
        #     return x

        # def sdf_function(pts):
        #     inv_pts = inverse_normalize(pts)
        #     sdf = self.gaussians.query_sdf(inv_pts)
        #     return sdf

        # bounding_box = (normalize(self.gaussians.bounding_box.permute(1,0))).cpu().numpy()

        # mesh = marching_cubes_with_contraction(
        #     sdf=sdf_function,
        #     bounding_box_min=bounding_box[0],
        #     bounding_box_max=bounding_box[1],
        #     level=0,
        #     resolution=N,
        #     inv_contraction=inverse_normalize,
        # )
        ...
        mesh = marching_cubes_with_contraction(
            sdf=sdf_function,
            bounding_box_min=(-R, -R/2, -R),
            bounding_box_max=(R, R/2, R),
            level=0,
            resolution=N,
            inv_contraction=inv_contraction,
        )
        ...
        torch.cuda.empty_cache()
        print("texturing mesh ... ")
        rgbs, mask = colorize(torch.tensor(np.asarray(mesh.vertices)).float().cuda(), inv_contraction=None)
        face_mask = mask[mesh.faces].all(axis=1)
        mesh.update_vertices(mask)
        mesh.update_faces(face_mask)
        mesh = mesh.as_open3d
        mesh.vertex_colors = o3d.utility.Vector3dVector(rgbs[mask])
        return mesh
