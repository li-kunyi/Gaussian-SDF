...
def sample_along_rays(gt_depth, n_samples, n_surface, device, truncation=0.05):
    # [N, C]
    ...
    z_vals_surface_depth_none_zero = (gt_depth_surface - truncation) * (1.-t_vals_surface) + (gt_depth_surface + truncation) * (t_vals_surface)
    ...
    near = torch.min(gt_depth) * 0.01
    far = torch.max(gt_depth) * 1.1
    t_vals_surface = torch.rand(n_surface).to(device)
    ...
    if n_samples > 0:
        ...
        t_vals = torch.rand(n_samples).to(device)
