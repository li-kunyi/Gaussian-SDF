...
class SimpleSDF(nn.Module):
    def __init__(self, cfg, bounding_box, in_dim=3, hidden_dim=32):
        ...
        if self.use_color:
            ...
            dir_dim = 3
            # self.color_decoder = tcnn.Network(n_input_dims=hidden_dim + dir_dim,
            #                                 n_output_dims=3,
            #                                 network_config={
            #                                     "otype": "CutlassMLP", # use CutlassMLP if not support FullyFusedMLP
            #                                     "activation": "ReLU",
            ...
            #                                     "n_neurons": hidden_dim,
            #                                     "n_hidden_layers": 1})
            # self.ref_decoder = tcnn.Network(n_input_dims=hidden_dim + dir_dim,
            #                                 n_output_dims=3,
            #                                 network_config={
            #                                     "otype": "CutlassMLP", # use CutlassMLP if not support FullyFusedMLP
            #                                     "activation": "ReLU",
        ...
        # self.decoder = tcnn.Network(n_input_dims=n_input_dims,
    ...
    def forward(self, x, view_dir=None, ref_dir=None, batch=50000):
        num = x.shape[0]
        out = []
        color = []
...
class LaplaceDensity(Density):  # alpha * Laplace(loc=0, scale=beta).cdf(-sdf)
    ...
    def density_func(self, sdf, beta=None):
        ...
        # return torch.exp(-(sdf / 0.5) ** 2)
        # return torch.exp(-(sdf / beta) ** 2) * torch.sigmoid(2 * sdf) * 2
