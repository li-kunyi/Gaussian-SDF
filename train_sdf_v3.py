...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration > 1000:
            fs_loss, sdf_loss, surface_normal_loss, normal_dir_loss, eikonal_loss = get_sdf_loss_with_gaussian_depth(gaussians, viewpoint_cam, 
        ...
        if iteration > 600:
            loss += 0.00001 * smoothness(gaussians)
        loss.backward()
        ...
        if is_save_images and (iteration % opt.densification_interval == 0):
            with torch.no_grad():
                ...
                rendering = sdf_render_v2(eval_cam, gaussians, pipe, background, kernel_size=dataset.kernel_size)["render"]
                image = rendering[:3, :, :]
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, rgb_loss, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, sdf_render_v2, (pipe, background, dataset.kernel_size))
            ...
            if iteration > 200 and iteration % 10000 == 0:
                ...
                grid = torch.stack((grid_x, grid_y, grid_z), dim=-1).cuda()

                grid_flat = grid.reshape(-1, 3)
                sdf_flat = gaussians.query_sdf(grid_flat)
                sdf_grid = sdf_flat.reshape(grid.shape[:3])
                sdf_grid = sdf_grid.detach().cpu().numpy()

                print('Running Marching Cubes')
                verts, faces, normals, values = marching_cubes(sdf_grid, level=0.0)
                # verts, faces = mcubes.marching_cubes(sdf_grid, 0.0, truncation=0.02)
...
def prepare_output_and_logger(args):    
    ...
    if TENSORBOARD_FOUND:
        ...
        ################################################################################
        ################################################################################
        tb_writer.add_text('Debug', 
                           'PE: oneblob, Embd: HashGrid /// ' +
                           'MLP output: SDF /// ' +
                           'SDF2Opacity: gaussian function /// ' +
                           'Loss: gaussian sdf loss, sdf loss, surface normal loss,  /// ' +
                           'MC th: 0. /// ', 
                           0)
        ################################################################################
        ################################################################################
...
def training_report(tb_writer, iteration, rgb_loss, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
        ...
        tb_writer.add_scalar('sdf2opacity_beta', scene.gaussians.sdf2opacity.get_beta(), iteration)
        tb_writer.add_scalar('sdf2opacity_alpha', scene.gaussians.sdf2opacity.get_alpha(), iteration)
    ...
    if iteration % 1000 == 0:
        ...
        if tb_writer:
            ...
            # free_energy = 2 * min_scales[:, None] * density
            # opacity = 1 - torch.exp(-free_energy)  # probability of it is not empty here
            tb_writer.add_histogram("scene/sdf_histogram", gaussian_sdf, global_step=iteration)
            # tb_writer.add_histogram("scene/opacity_histogram", opacity, global_step=iteration)
            tb_writer.add_histogram("scene/density_histogram", density, global_step=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[2_000, 4_000, 6_000, 8_000, 10_000, 20_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 6_000, 9_000, 12_000, 30_000])
