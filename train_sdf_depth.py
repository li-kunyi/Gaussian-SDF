...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, save_ckpt, ckpt_pth, debug_from):
    ...
    num_trainCameras = len(trainCameras)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        cur_name = viewpoint_cam.image_name
        cur_idx = viewpoint_cam.idx
        ...
        if 'dtu' in dataset.source_path:
            mask = Image.open(os.path.join(dataset.source_path, f'mask/{cur_name[1:]}.png'))
            mask = np.array(mask)[:, :, 0] / 255
            mask = F.interpolate(torch.from_numpy(mask).unsqueeze(0).unsqueeze(0), size=(H, W), mode='bilinear', align_corners=False).squeeze(0).squeeze(0)
            inf_mask = ~mask.bool().cuda()
            inf_mask = torch.logical_or(inf_mask, (gt_depth_map < 0.0001))
            gt_depth_map = 1 / (gt_depth_map + 1e-10)
            gt_depth_map[inf_mask] = 0.
        else:
            inf_mask = (gt_depth_map < 0.0001)
            gt_depth_map = 1 / (gt_depth_map + 1e-10)
            
        iter_start.record()

        if cur_idx not in align.keys():
            align[cur_idx] = torch.nn.Parameter(torch.tensor([1, 0], device="cuda", dtype=torch.float))
        align_optimizer = torch.optim.Adam([align[cur_idx]], lr=0.1)
        scale = align[cur_idx][0]
        shift = align[cur_idx][1]
        ...
        lambda_gradient_consistency = opt.lambda_gradient_consistency if iteration >= opt.start_train_sdf else 0.0
        loss += lambda_gradient_consistency * consistency_loss

        # depth loss
        valid_mask = (depth < 6.0).detach()
        aligned_depth = scale * gt_depth_map + shift
        depth_loss = (torch.log(1 + torch.abs(depth - aligned_depth)))[valid_mask].mean()
        ...
        normal_loss = (torch.abs(1 - (render_normal_world * gt_depth_normal).sum(dim=0)))[valid_mask].mean()
        ...
        if iteration > opt.start_train_sdf:
            ...
            lambda_fs = 1.0 if iteration < opt.densify_until_iter else opt.lambda_fs
        ...
        with torch.no_grad():
            ...
            if is_save_images and (iteration % 1000 == 0):            
                depth_map = apply_depth_colormap(depth[..., None], None, near_plane=0.1, far_plane=32)
                depth_map = depth_map.permute(2, 0, 1)

                aligned_depth_map = apply_depth_colormap(aligned_depth[..., None], None, near_plane=0.1, far_plane=32)
                aligned_depth_map = aligned_depth_map.permute(2, 0, 1)
                ...
                _gt_depth_normal = (gt_depth_normal + 1) / 2
            
                row0 = torch.cat([gt_image, image, depth_map, aligned_depth_map], dim=2)
                row1 = torch.cat([colored_accum_alpha, _render_normal_world, _depth_normal, _gt_depth_normal], dim=2)
            ...
            if iteration < opt.densify_until_iter and iteration > opt.densify_from_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and visibility_filter.shape[0] < 5e6:
                    size_threshold = None
                    start_prune = False if iteration < num_trainCameras else True
                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.05, scene.cameras_extent, size_threshold, opacity=render_pkg["opacity"], start_prune=start_prune)
                    gaussians.compute_3D_filter(cameras=trainCameras)
