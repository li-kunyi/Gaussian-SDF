...
def sdf_render_v3(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size: float, scaling_modifier = 1.0, subpixel_offset=None):
    ...
    gaussian_sdf = pc.query_sdf(means3D.detach())
    gaussian_opacity = pc.opacity_activation(gaussian_sdf)
    opacity = gaussian_opacity * pc.get_opacity_with_3D_filter
...
def get_sdf_loss_with_gaussian_depth(gaussians, c2w, fx, fy, depth,
    ...
    volume_fs_loss = (((sdf * front_mask - torch.ones_like(sdf) * front_mask) ** 2)).mean()
    volume_sdf_loss = ((((z_vals + sdf) * sdf_mask - depth_sp * sdf_mask) ** 2)).mean()

    return volume_fs_loss, volume_sdf_loss
...
def get_mv_loss(camera, depth_map, alpha_map, refer_camera, refer_depth_map, refer_alpha_map, device="cuda"):
    pts = depths_to_points(camera, depth_map[None, ...])
    # for each gaussian, project to image coordinate to match depth
    H, W = refer_camera.image_height, refer_camera.image_width
    K = torch.eye(3).to(device)
    K[0, 0] = W / (2 * math.tan(refer_camera.FoVx / 2))
    K[1, 1] = H / (2 * math.tan(refer_camera.FoVy / 2))
    K[0, 2] = (W - 1) / 2
    K[1, 2] = (H - 1) / 2
    rel_w2c = refer_camera.world_view_transform.T
    pts_ones = torch.ones(pts.shape[0], 1).cuda().float()
    pts4 = torch.cat((pts, pts_ones), dim=1)
    transformed_pts = (rel_w2c @ pts4.T).T[:, :3]
    u, v, d = transformed_pts[:, 0], transformed_pts[:, 1], transformed_pts[:, 2]
    ...
    valid_indices = (image_coordinates[:, 0] > 10) & (image_coordinates[:, 0] < W-10) & \
                    ...
                    (d > 0) & (alpha_map.reshape(-1) > 0.5)
    valid_coordinates = torch.round(image_coordinates[valid_indices, :]).long()
    proj_depth = d[valid_indices]

    refer_depth = refer_depth_map[valid_coordinates[:, 1], valid_coordinates[:, 0]]
    refer_alpha = refer_alpha_map[valid_coordinates[:, 1], valid_coordinates[:, 0]]

    mv_loss = torch.abs((proj_depth - refer_depth) * (refer_alpha > 0.5)).mean()
    return mv_loss
